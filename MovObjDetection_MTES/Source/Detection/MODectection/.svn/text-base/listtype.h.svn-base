...
//	노드들을 연결하여 관리하는 List class(List)를 정의. List classs ADT은 다음과 같다. 
template <class Type> 
class ListType
{
public:
	ListType(); //생성자: first를 0으로 초기화
	~ListType(); // 연결된 모든 노드를 삭제한다. 
	void AddFront(Type item);	// 수정 2003.07.14 kellt -> item을 List의 맨앞에 추가
	void AddEnd(Type item);  // item을 List의 맨뒤에 추가
	void ResetCurNode();  // curNode pointer를 첫 노드로 세팅
	int GetNextItem(Type& data); // curNode가 가리키는 노드의 자료를 리턴하고 curNode를 다음 노드로 링크. 
								 //리스트 end에 도달하면 null을 리턴
	void PrintList(); // list의 모든 노드들을 화면에 출력한다.

	long GetLength(void);
	Type get(int index);	//수정 2003.07.14 kellt-> n번째 data 갖고 오기
...
private:
	NodeType<Type> *first; // 연결리스트의 시작을 가리킨다
	NodeType<Type> *last; // 연결리스트의 끝을 가리킨다
	NodeType<Type> *curNode; // 현재 노드 포인터
